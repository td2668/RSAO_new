...
$tmpl=loadPage("hrimport", 'HR Import');
           ...
           rtrim($record,'\n');
            if($key == 0){
               ...
               //print_r($keys);
                ...
                //$temp=explode(',',$record);
				$temp=str_getcsv($record);
                $records[]=array_combine($keys,$temp);
            } //if not initial record
       } //foreach
       foreach($records as $key=>$record){
	   		//if($key=='SURNAME' || $key=='PREFRD_NAME' || $key=='NT_USERNAME') $records[$key]=ucwords(strtolower($record);
	   		$records[$key]['SURNAME'] = ucwords(strtolower($record['SURNAME']));
	   		$records[$key]['PREFRD_NAME'] = ucwords(strtolower($record['PREFRD_NAME']));
	   		$records[$key]['NT_USERNAME'] = strtolower($record['NT_USERNAME']);
	   }
...
if (isset($_REQUEST['section'])) {
	...
	switch($_REQUEST['section']){
		case "load":          
            ...
            $tmpl->setAttribute("load","visibility","visible");
            $hdr->AddVar("header","title","HR Import: Load");
