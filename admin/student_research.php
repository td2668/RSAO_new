...
function view() {
    ...
    $sql="SELECT project.projectUrl,project.id, GROUP_CONCAT(student.first, ' ', student.last ORDER BY student.aorder SEPARATOR ' , ' ) as name, project.title,
        ...
        GROUP BY project.id
        ORDER BY project.hidden desc, project.title";
        //project.projectUrl,project.startDate desc,project.title
    global $db;
    ...
    if(count($projects)>0){
        foreach($projects as $key=>$project){
            ...
            $projects[$key]['url']=(strlen($project['projectUrl'])>1) ? "checked='checked'" : ''; 
...
function edit() {
    if(isset($_REQUEST['id'])) {
        ...
        if($project){
            ...
            $project['presentationTypeMM']=($project['presentationType'] == "Multimedia") ? "checked='checked'" : '';
...
function getStudentResearchers($db, $projectID)
{
    $sql = sprintf("SELECT student.id, CONCAT(student.last, ', ', student.first)  as name FROM student_researchers AS student
                    ...
                    ORDER BY student.aorder,student.last", (int)$projectID);
...
function getFacultyDropDown($db, $supervisorID)
{
    $facultySql = "SELECT user_id, CONCAT(users.last_name, ', ', users.first_name) as name
                            FROM users
                            WHERE emp_type = 'FACL' AND inactive_flag = 0
                            ORDER BY name";
