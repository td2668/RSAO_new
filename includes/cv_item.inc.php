...
function GenerateEditForm ( $cvItemId, $page, $userId, &$tmpl ) {
    ...
    $projects= getProjects($userId, $cvItemId);
    $tmpl->addRows('PROJECTS', $projects);

    require_once('includes/user_functions.php');
    $allusers = getUsers();
    $tmpl->addRows('USER_OPTIONS', $allusers);
    ...
    if ( !$cvItemId && $typeId ) {
        ...
        $sql = "INSERT INTO cas_cv_items (cas_type_id, user_id, f2, f3, web_show, mycv1, rank)
            VALUES ({$typeId}, {$userId}, {$insertdate}, {$insertdate}, 1, 1,{$nextRank})";
        $db->Execute( $sql );
    ...
    if ( $typeId && sizeof( $item ) > 0 ) {
        ...
        $tmpl->setAttribute("boyers", "visibility", "show");
        $tmpl->setAttribute("realtedto", "visibility", "show");
        $tmpl->setAttribute("toolssection", "visibility", "show");
        $tmpl->setAttribute("associatedprojects", "visibility", "show");
    ...
    // do details for 5boyer's categories

    $tmpl->addVar( 'boyers', 'BOYERDISCOVERY', $item['boyerDiscovery'] == 1 ? 'checked' : '' );
    $tmpl->addVar( 'boyers', 'BOYERINTEGRATION', $item['boyerIntegration'] == 1 ? 'checked' : '' );
    $tmpl->addVar( 'boyers', 'BOYERAPPLICATION', $item['boyerApplication'] == 1 ? 'checked' : '' );
    $tmpl->addVar( 'boyers', 'BOYERTEACHING', $item['boyerTeaching'] == 1 ? 'checked' : '' );
    $tmpl->addVar( 'boyers', 'BOYERSERVICE', $item['boyerService'] == 1 ? 'checked' : '' );
...
/**
 * Get the user projects for use in a drop-down on the CV item edit form
 *
 * @param $userId - the userID
 * @param $cvItemId - the cvItemId of the CV item that is being edited
 * @return array('project_id'       => the project ID
 *               'name'             => the name of the project
 *               'project_selected' => whether the program is selected for this CV item.
 *              )
 */
function getProjects($userId, $cvItemId)
{
    global $db;

    require_once("includes/user_functions.php");
    $projects = getUserProjects($userId); // get user's projects for drop-down

    $sql = sprintf("SELECT `project_id` FROM projects_cvitems WHERE cv_item_id = %s", $cvItemId);
    $associatedProjectId = $db->getRow($sql); // determine which project is associated with this item


    if (count($associatedProjectId) > 0) {
        foreach ($projects as $key=> $project) {
            if ($project['project_id'] == $associatedProjectId['project_id']) {
                $projects[$key]['project_selected'] = 'selected';
            }
        }
    }
...
function SaveForm ( $cvItemId, $userId, &$tmpl ) {
    ...
    if ( $item ) {
        ...
        //where we check for the details for boyer's categories
        $boyerDiscovery = $_POST['boyerDiscovery'] == 1 ? 1 : 0;
        $boyerIntegration = $_POST['boyerIntegration'] == 1 ? 1 : 0;
        $boyerApplication= $_POST['boyerApplication'] == 1 ? 1 : 0;
        $boyerTeaching = $_POST['boyerTeaching'] == 1 ? 1 : 0;
        $boyerService = $_POST['boyerService'] == 1 ? 1 : 0;

        $fields[] = "boyerDiscovery = " . $boyerDiscovery;
        $fields[] = "boyerIntegration = " . $boyerIntegration;
        $fields[] = "boyerApplication = " . $boyerApplication;
        $fields[] = "boyerTeaching = " . $boyerTeaching;
        $fields[] = "boyerService = " . $boyerService;
        ...
        if ( $db->Execute( $sql ) == false ) {
            $statusMessage .= 'Sorry, an error occurred (update query failed).';
            echo $sql;
        } else {
        ...
        // save the associated project, if one was selected
        $projectId = $_POST['projectAssociation'];
        saveProjectAssociation($cvItemId, $projectId);
...
/**
 * Save the project association
 *    - if $projectId == 0 then delete all associations
 *    - if entry already exists, then do an update
 *    - otherwise insert a new record
 *
 * @param $cvItemId - the CV item id
 * @param $projectId - the project ID
 */
function saveProjectAssociation($cvItemId, $projectId) {
    global $db;

    try {
        if($projectId == 0) {
            $sql = sprintf("DELETE FROM projects_cvitems WHERE cv_item_id = %s", $cvItemId);
            $db->Execute($sql);
        } else {
            $sql = sprintf("UPDATE projects_cvitems SET `project_id` = %s WHERE `cv_item_id` = %s", $projectId, $cvItemId);
                $db->Execute($sql);
            if(mysql_affected_rows() == 0) {
                $sql = sprintf("INSERT into projects_cvitems (project_id, cv_item_id)
                            VALUES(%s, %s)", $projectId, $cvItemId);
                $db->Execute($sql);
            }
        }
    } catch(Exception $e) {
