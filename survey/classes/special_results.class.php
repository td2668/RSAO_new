...
class UCCASS_Special_Results extends UCCASS_Main
{
    ...
    function results_table($sid)
    {
        ...
        $query = "SELECT GREATEST(IFNULL(rt.qid,''),      IFNULL(r.qid,'')) AS qid,
GREATEST(IFNULL(rt.sequence,''), IFNULL(r.sequence,'')) AS seq,
GREATEST(IFNULL(rt.entered,''),  IFNULL(r.entered,'')) AS entered,
    ...
    function results_csv($sid, $export_type=EXPORT_CSV_TEXT)
    {
        ...
        $query = "SELECT GREATEST(IFNULL(rt.qid,''), IFNULL(r.qid,'')) AS qid, GREATEST(IFNULL(rt.sequence,''), IFNULL(r.sequence,'')) AS seq,
                  GREATEST(IFNULL(rt.entered,''), IFNULL(r.entered,'')) AS entered,
